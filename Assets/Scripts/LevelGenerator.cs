...
public class LevelGenerator : MonoBehaviour
{
    ...
    public void GenerateRoom(TileType[,] _tiles)
    {
        ...
        for (int i = 0; i < screenHeight/tileSize; i++)
        {
            for (int j = 0; j < screenWidth/tileSize; j++)
            {
                ...
                SpriteRenderer newTile = Instantiate(tilePrefab, tilePosition, Quaternion.identity).GetComponent<SpriteRenderer>();
                newTile.sprite = GetSprite(_tiles[i, j]);
            }
        }
    }

    private Sprite GetSprite(TileType _type)
    {
        switch (_type)
        {
            case TileType.Floor:
